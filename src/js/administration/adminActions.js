...
export const updateMascota = async (dataJson) => {
    ...
    let response = await fetch(BASE_URL_ADOPTIONS + `/Mascota/${dataJson.mascotaId}`, settings).catch(handleError)
    let data = await response.json()
...
export const deleteMascota = async (id) => {
    ...
    let response = await fetch(BASE_URL_ADOPTIONS + `/Mascota/${id}`, settings).catch(handleError)
