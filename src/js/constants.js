export const ROL_ADMIN = 1;
export const ROL_VETERINARIO = 2;
export const ROL_CLIENTE = 3;

//DEFINIR URLS ACA
//MICROSERVICIO HISTORIA CLINICA
export const URL_API_CLIENTE = 'https://localhost:44314/api/Cliente';
export const URL_API_TURNO = 'https://localhost:44314/api/Turno';
export const URL_API_HISTORIA_CLINICA ='https://localhost:44314/api/HistoriaClinica';
export const URL_API_REGISTROS = 'https://localhost:44314/api/Registros';
export const URL_API_TRATAMIENTO = 'https://localhost:44314/api/Tratamiento';
export const URL_API_MASCOTA = 'https://localhost:44314/api/Mascota';
export const URL_API_REGISTER = 'https://localhost:44315/api/register';
